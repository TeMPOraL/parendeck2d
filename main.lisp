...
(defun run-main-loop ()
  ...
  (let ((dt 0)
        ...
        (frames-counter (p2dprof:get-counter 'p2d-frames :interval 1 :history-size 10)))
      ...
      (:idle ()
             ...
             (when *use-fixed-timestep*
               ...
               (loop while (> dt-accumulator *update-step*) do
                    (p2dprof:with-profiling ('p2d-game-tick :description "main loop single tick" :interval 0 :history-size 128)
                        (on-tick *game* *update-step*))
                    (decf dt-accumulator *update-step*)))

             (on-idle *game* dt)
             (p2dprof:with-profiling ('p2d-on-render :description "main loop on-render" :interval 0 :history-size 64)
              (on-render *game* dt))
